...
from tools.cache_deco import topic_cache
...
class TopicView(View):
    ...
    def clear_topic_caches(self, request):
        ...
        # 3. 后缀是用了区分分类的【'','?category=tec','?category=no-tec'】
        part3 = ['', '?category=tec', '?category=no-tec']
        # 4. 三部分组合到一起，添加到一个key的列表中
        all_keys = []
        for p1 in part1:
            for p3 in part3:
    ...
    def get(self, request, author_id):
        print('deaffffffffffffffffffffffffffffffffffffffffffff')
